...
def dynamic_speedbump(transaction_time):
    global speedbump_sum
    global order_count

    speedbump = -1*transaction_time + 1/MAX_ORDERS
    speedbump_sum += speedbump
    sleep(speedbump_sum/order_count)
        ...
        while (tick > 5) and (tick < 295) and not shutdown:
            ...
            if (open_sells_volume == 0) and (open_buys_volume == 0):
                ...
                if spread >= SPREAD:
                    ...
                    transaction_time = start - time.time()
                    dynamic_speedbump(transaction_time)
